...
class Lunar {
    ...
    public function getHoliday($year,$format=''){

        $Holiday = [];
        self::set3Day($Holiday,strtotime($year.'/01/01'),'元旦');

        $dates = self::convertLunarToSolar($year-1,12,self::getLunarMonthDays($year,12));
        self::set7Day($Holiday,strtotime($dates[0].'/'.$dates[1].'/'.$dates[2]),'春节');

        self::set3Day($Holiday,strtotime(self::qingming($year)),'清明');

        self::set3Day($Holiday,strtotime($year.'/05/01'),'劳动');

        $dates = self::convertLunarToSolar($year,5,5);
        self::set3Day($Holiday,strtotime($dates[0].'/'.$dates[1].'/'.$dates[2]),'端午');

        $dates = self::convertLunarToSolar($year,8,15);
        self::set3Day($Holiday,strtotime($dates[0].'/'.$dates[1].'/'.$dates[2]),'中秋');

        self::set7Day($Holiday,strtotime($year.'/10/01'),'国庆');
        if($format){
            foreach ($Holiday as &$v){
                $v['date'] = date($format,$v['date']);
            }
        }
        return array_values($Holiday);
    }

    protected function set3Day(&$Holiday,$time,$title){
        $w = date('w',$time);
        switch ($w){
            case 0:
                $Holiday[$time-86400] = ['title'=>$title,'type'=>0,'date' =>$time-86400];
                $Holiday[$time] = ['title'=>$title,'type'=>0,'date' =>$time];
                $Holiday[$time+86400] = ['title'=>$title,'type'=>0,'date' =>$time+86400];
                break;
            case 1:
            case 2:
                $Holiday[$time-86400*2] = ['title'=>$title,'type'=>0,'date' =>$time-86400*2];
                $Holiday[$time-86400] = ['title'=>$title,'type'=>0,'date' =>$time-86400];
                $Holiday[$time] = ['title'=>$title,'type'=>0,'date' =>$time];
                break;
            case 3:
    ...
    protected function set7Day(&$Holiday,$time,$title){
        $Holiday[$time] = ['title'=>$title,'type'=>0,'date' =>$time];
        $Holiday[$time+86400] = ['title'=>$title,'type'=>0,'date' =>$time+86400];
        $Holiday[$time+86400*2] = ['title'=>$title,'type'=>0,'date' =>$time+86400*2];
        $Holiday[$time+86400*3] = ['title'=>$title,'type'=>0,'date' =>$time+86400*3];
        $Holiday[$time+86400*4] = ['title'=>$title,'type'=>0,'date' =>$time+86400*4];
        $Holiday[$time+86400*5] = ['title'=>$title,'type'=>0,'date' =>$time+86400*5];
        $Holiday[$time+86400*6] = ['title'=>$title,'type'=>0,'date' =>$time+86400*6];
    }
     ...
     * @return string
     */
    public function midwinter($year){
        $Y = substr($year, -2);
        $L= floor($Y/4);
        $D=0.2422;
        $C = $year>1999?21.94:22.60;
        $d =($Y*$D+$C)-$L;
        if($year==1918||$year==2021){
            $d--;
        }

        return $year.'/12/'.floor($d);
    }
     ...
     * @return string
     */
    public function qingming($year){
            $midwinter = self::midwinter($year-1);
            return date('Y/m/d',strtotime($midwinter)+104*86400);
        }
