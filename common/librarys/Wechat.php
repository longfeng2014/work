 ...
 * @version $Id: Wechat.php 21025 2016-05-05 10:26:02Z A1292 $
 */
...
class Wechat
{
    ...
    const EVENTTYPE_MASSSENDJOBFINISH = 'MASSSENDJOBFINISH'; //新增群发消息完成事件 
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    public function uploadArticles($data){
        ...
        foreach ($data as &$item){
            foreach ($item as $k=>$v){
                ...
                if($k =='content'){
                    $v = str_replace('src="/uploads/', 'src="'.CDN_URL.'uploads/', $v);
     ...
     * 注意：unionid字段 只有在用户将公众号绑定到微信开放平台帐号后，才会出现。建议调用前用isset()检测一下
     */
     ...
     *"kf_online_list": [
     *{
     *"kf_account": "test1@test",    //客服帐号@微信别名
